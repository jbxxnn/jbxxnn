...
import { useMousePositionRef } from "@/hook/use-mouse-position-ref"
  ...
  (
    ...
    const renderText = (text: string, keyPrefix: string) =>
      text.split(/(\s+)/).map((segment, segmentIndex) => {
        ...
        if (/^\s+$/.test(segment)) {
          return (
            <span key={`${keyPrefix}-space-${segmentIndex}`} aria-hidden="true">
              {segment}
            </span>
          )
        }
    ...
    const renderNode = (node: React.ReactNode, keyPrefix = "node"): React.ReactNode => {
      if (typeof node === "string" || typeof node === "number") {
      ...
      if (isValidElement(node)) {
        ...
        return React.cloneElement(element, {
          ...
          children: renderNode(element.props.children, `${keyPrefix}-child`),
        })
      }
